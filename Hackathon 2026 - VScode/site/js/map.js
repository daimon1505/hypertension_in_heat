...
window.NycMap = (function () {
  ...
  const FILL = { safe: "#d8d4ce", moderate: "#f7b87c", dangerous: "#e2600a", none: "#eceae6" };
  const openMaps = [];
  ...
  function draw(el, opts) {
    ...
    const layer = L.geoJSON(data.neighborhoods, {
      style: (f) => {
        ...
        return { color: "#ffffff", weight: 0.9, fillColor: band ? FILL[band.id] : FILL.none, fillOpacity: opts.fillOpacity || 0.9 };
      },
      onEachFeature: (f, lyr) => {
        const band = opts.bandFor(f.properties);
